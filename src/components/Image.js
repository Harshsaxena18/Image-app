...
const ImageScreen = () => {
  ...
  async function query(imageData, wait_for_model = true ) { // Add wait_for_model parameter
    ...
    // Handle 503 error with retry logic (if API doesn't support wait_for_model)
    if (response.status === 503 && wait_for_model) {
      console.warn('API overloaded (503). Retrying in 2 seconds...');
      await new Promise((resolve) => setTimeout(resolve, 2000)); // Wait for 2 seconds
      return query(imageData, true); // Retry with wait_for_model (if supported)
    }
